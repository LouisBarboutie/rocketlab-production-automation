...
class Server(QObject):
    ...
    def __init__(self) -> None:
        ...
        self.active_commands: Dict[Device, Command] = {}

    @pyqtSlot(Device, Command)
    def command(self, device: Device, command: Command):
        if device in self.active_commands:
            logging.error(
                f"Command for device {device} is already being processed, cannot execute {command}"
            )
            return
        ...
        self.active_commands[device] = command
        thread.start()
    ...
    def do_transaction(self, address: str, port: int, command: Command):
        ...
        self.active_commands.pop(device)
